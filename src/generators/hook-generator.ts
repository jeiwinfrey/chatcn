...
/**
 * Generates hook files for a template by copying hook files from the registry
...
export async function generateHookFiles(
  ...
  // Filter template files by type === 'hook'
  const hookFiles = template.files.filter((file) => file.type === "hook");
